...
func (s *Server) handleTestGitCredential(w http.ResponseWriter, r *http.Request) {
	var req testGitCredentialRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		s.writeError(w, http.StatusBadRequest, "invalid request body")
		return
	}
	...
	if req.ID != 0 && token == "" {
		storedUser, storedToken, err := s.gitcreds.Credentials(r.Context(), req.ID)
		if err != nil {
			s.writeError(w, http.StatusNotFound, "credential not found")
			return
		}
	...
	if repoURL == "" || username == "" || token == "" {
		s.writeError(w, http.StatusBadRequest, "repository url, username and token are required")
		return
	}

	if err := gitcred.TestAccess(r.Context(), repoURL, username, token); err != nil {
		if errors.Is(err, gitcred.ErrUnauthorized) {
			s.writeError(w, http.StatusBadGateway, "could not authenticate with the repository")
			return
		}
		s.logger.Warn("git credential test failed", "repo", repoURL, "err", err)
		s.writeError(w, http.StatusBadGateway, "could not reach the repository")
		return
	}
